...
const News = ({ category, country }) => {
  ...
  const fetchNews = async (url) => {
    ...
    let parsedData =
      data?.articles &&
      data.articles.filter((data, i, self) => {
      ...
      parsedData &&
        setArticles((prev) => {
          ...
          return prev.length
            ? prev.filter((data, i) => {
                return (
                  parsedData.findIndex((news) => news.url === data.url) === i
                )
              })
            : [...parsedData]
        })
  ...
  useEffect(() => {
    //* Fetching News
    fetchNews(
      `https://explorer.emupedia.net/?url=https://newsapi.org/v2/top-headlines?country=${country}&category=${category.toLowerCase()}&page=${page}&pageSize=${pagesize}`
    )
    }, [hasMore])
  ...
  useEffect(() => {
    if (search) {
      ...
      fetchNews(
        `https://explorer.emupedia.net/?url=https://newsapi.org/v2/everything?q=${searchTerm}&searchIn=${
  ...
  return (
    <>
      <div className="container pt-2 mb-5 pb-5">
        <h2
          className="text-center main-heading mb-3 mt-5"
        ...
        <div className="input-group flex-nowrap mb-2">
          ...
          <select
            ...
            aria-label={SEARCH_IN}
            value={searchIn}
            onChange={(e) => setSearchIn(e.target.value)}
          >
            <option>{SEARCH_IN}</option>
          ...
          >
            <option defaultValue="popularity">{SORT_BY}</option>
